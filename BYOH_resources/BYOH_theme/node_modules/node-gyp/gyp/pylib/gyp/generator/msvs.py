...
def _ConfigWindowsTargetPlatformVersion(config_data):
    ...
    for key in [
        ...
        names = sorted(
            [x for x in os.listdir(r"%s\include" % sdkdir) if x.startswith(version)],
            reverse=True,
        )
        return names[0]
    ...
    for cpy in spec.get("copies", []):
        for src in cpy.get("files", []):
            ...
            if src.endswith("/"):
                ...
                copies.append(
                    ([src], ["dummy_copies", dst], cmd, "Copying %s to %s" % (src, dst))
                )
            else:
                cmd = 'mkdir "%s" 2>nul & set ERRORLEVEL=0 & copy /Y "%s" "%s"' % (
                    _FixPath(cpy["destination"]),
                    _FixPath(src),
                    _FixPath(dst),
                )
...
def _MapFileToMsBuildSourceType(source, rule_dependencies, extension_to_rule_name):
...
def _GetMSBuildGlobalProperties(spec, guid, gyp_file_name):
    ...
    if spec.get("msvs_enable_winrt"):
        ...
        if spec.get("msvs_target_platform_version"):
            ...
            properties[0].append(
                ["WindowsTargetPlatformVersion", target_platform_version]
            )
    ...
    for configuration in spec["configurations"].itervalues():
        ...
        msvs_windows_target_platform_version = (
            msvs_windows_target_platform_version
            or _ConfigWindowsTargetPlatformVersion(configuration)
        )
    ...
    if msvs_windows_target_platform_version:
        properties[0].append(
            ["WindowsTargetPlatformVersion", str(msvs_windows_target_platform_version)]
        )
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
    ...
    _GenerateMSBuildFiltersFile(
        project.path + ".filters", sources, rule_dependencies, extension_to_rule_name
    )
    missing_sources = _VerifySourcesExist(sources, project_dir)
